...
!!! warning "`params`에 시크릿을 절대 넣지 마세요"
    시크릿처럼 보이는 키를 `params`에 절대 넣지 마세요 — 그 값은 기록됩니다. 시크릿은 `secret: true`와 함께 `scopes.config`로 선언하며, 웹에서는 데스크톱 플러그인으로 라우팅됩니다 — [Security](security.md)를 참조하세요. Korean Roulette은 시크릿과 네트워크가 필요 없기 때문에 깔끔한 첫 플러그인으로 적합합니다.
...
단위 테스트가 통과하면 실제 사용하는 앱 — [vineyard.run](https://vineyard.run/)의 웹 앱 또는 데스크톱 빌드 — 에서 실제 그래프를 대상으로 플러그인을 엔드 투 엔드로 실행하세요. 이를 위해 Vineyard 소스 코드나 로컬 개발 스택이 필요하지 않습니다: **Developer Mode**를 통해 번들을 사이드로드하고(반복 중에는 개발 서버 URL 모드가 이상적), 폐기용 프로젝트에서 실행을 트리거하면, `run(ctx)`이 실행될 때 노드와 엣지가 실시간으로 캔버스에 나타나거나 변경되거나 사라지는 것을 볼 수 있습니다. 이것은 게시 전 프로덕션 동작에 가장 가까운 방식입니다: 실제 쓰기, 실제 실행 토큰, 실제 WebSocket 팬아웃 — 단지 레지스트리 대신 로컬 번들에서 소싱될 뿐입니다.
