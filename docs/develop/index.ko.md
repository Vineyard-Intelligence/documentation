...
설치하면 Vineyard 앱이 고정된 커밋에서 귀하의 저장소로부터 직접 가져옵니다. 서버는
플러그인 코드를 실행하지 않고, 플러그인 바이트를 저장하지 않으며, 기본적으로
플러그인이 생성하는 실행을 기록하지 않습니다.
...
!!! note "초기 범위는 브라우저 + 데스크톱"
    **브라우저** 런타임(`platforms.web.runtime: "sandbox-js"`)과 **데스크톱** Electron
    셸(`platforms.desktop.runtime: "sandbox-js"`) 모두 현재 배포되어 있으며, 데스크톱의
    키체인 기반 시크릿 `config`도 포함됩니다. `web-proxy` CORS 우회와 `native`/`subprocess`
    데스크톱 런타임은 스키마에 미래 지향적 설계로 존재하지만 **연기**된 상태로, 아직
    빌드되지 않았습니다.

## 다음 단계

- [Quickstart](quickstart.md) — 스캐폴딩, 번들링, 로컬에서 플러그인 테스트.
- [Plugin manifest](plugin-manifest.md) — 실제 예제와 함께 모든 필드 설명.
