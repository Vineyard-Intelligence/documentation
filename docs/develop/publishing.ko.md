...
병합되면 `packs/`에서 세 카탈로그 파일이 재생성되어 `main`에 바로 커밋됩니다 — GitHub Pages가 브랜치를 그대로 서빙하므로 게시되는 바이트가 트리에 존재해야 하기 때문입니다. 다음에 클라이언트가 레지스트리를 가져올 때 귀하의 항목이 파생 배지와 함께 찾아보기에 나타납니다.
